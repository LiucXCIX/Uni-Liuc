...
public class Queen implements Pieces {
    ...
    */
    ...
    /**
     * Post-condizioni: inizializza e restituisce un'istanza di Queen
     * @param isBlack definisce se la regina è nera o meno
     */
    public Queen(boolean isBlack){
        if (isBlack) {
            ChessSquare = ChessCoordinate.coord(7, 3);
            this.image = "\u265B";
        } else {
            ChessSquare = ChessCoordinate.coord(0, 3);
            this.image = "\u2655";
        }
    ...
    /**
     * Post-condizioni: inizializza e restituisce un'istanza di Queen (viene principalmente usata per le promozione dei pedoni)
     * @param isBlack definisce se la regina è nera o meno
     * @param square la posizione dove verrà creata la regina
     */
    public Queen (boolean isBlack, ChessCoordinate square) {
    ...
    public boolean move(ChessCoordinate move, List<Pieces> AlivePieces) throws InvalidMoveException {
        if (isPinned) throw new InvalidMoveException("The piece is pinned");
    ...
    public boolean possibleMoves(ChessCoordinate move, List<Pieces> AlivePieces) {
        ...
        for(int i = -8; i < 8; i++){
            if (((move.xCoord() == ChessSquare.xCoord() + i) && (move.yCoord() == ChessSquare.yCoord()))) return true;
            if (((move.xCoord() == ChessSquare.xCoord() + i) && (move.yCoord() == ChessSquare.yCoord() + i))) return true;
            if (((move.xCoord() == ChessSquare.xCoord()) && (move.yCoord() == ChessSquare.yCoord() + i))) return true;
            if (((move.xCoord() == ChessSquare.xCoord() - i) && (move.yCoord() == ChessSquare.yCoord() + i))) return true;
        }
        return false;
    }
